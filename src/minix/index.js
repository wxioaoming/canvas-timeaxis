...
// var timestamps = new Date().setHours(0, 0, 0, 0); //当前时间戳
// var andTime = current_time - timestamps
// var start_timestamp = current_time - andTime;
...
function add_graduations(start_timestamp){
    ...
    for(var i = 0; i < num_steps; i++){
        ...
        // console.log('graduation_left====',graduation_left);
...
function add_events(){
    canvas.addEventListener('mousewheel',mousewheelFunc,{ passive:false });       //滚动
    canvas.addEventListener('mousedown',mousedownFunc,false);                     //点击
    canvas.addEventListener('mousemove',mousemoveFunc,false);                     //移入
    canvas.addEventListener('mouseup',mouseupFunc,false);                         //鼠标离开时
    canvas.addEventListener('mouseout',mouseoutFunc,false);                       //鼠标移出canvas时
}
